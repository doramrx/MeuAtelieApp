...
} from "../CreateOrder/AdjustService";
        ...
        transaction.executeSql(
          ...
          (_, resultSet) => {
            ...
            const orderData = {
              ...
              customerPhone: resultSet.rows.item(0).customer_phone,
              orderCost: resultSet.rows.item(0).order_cost,
              orderHiredAt: new Date(resultSet.rows.item(0).order_hired_at),
              orderDueDate: new Date(resultSet.rows.item(0).order_due_date),
              items: [],
            ...
            resultSet.rows._array.forEach((row) => {
              if (orderData.items.length === 0) {
                orderData.items.push({
                  ...
                  orderItemDescription: row.order_item_description,
                  adjusts: [
                    {
                      orderedServiceId: row.ordered_service_id,
                      orderedServiceCost: row.ordered_service_cost,
                      adjustServiceId: row.adjust_service_id,
                      adjustDescription: row.adjust_service_description,
                    },
                  ],
                });
                return;
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.mainContainer}>
        <ScrollView
          style={[styles.mainContainerWrapper, { paddingHorizontal: 0 }]}
        >
          {mode === "detail" ? (
  ...
  return (
    <Fragment>
      ...
      <ClientInfo
        ...
        phone={props.orderDetails ? props.orderDetails.customerPhone : ""}
      ...
      {props.orderDetails &&
        props.orderDetails.items.map((item, index) => {
          return (
            <OrderItem
              key={index}
              itemName={`Peça (${(index + 1).toString().padStart(2, "0")})`}
              data={item}
              isExpanded={orderItemsState ? orderItemsState[index] : false}
              isEditable={false}
      ...
      <View
        style={[styles.infoContainer, { paddingHorizontal: 25, marginTop: 20 }]}
      >
        <Text style={styles.infoLabel}>Contratado em:</Text>
        <Text style={styles.infoText}>
          {props.orderDetails
            ? props.orderDetails.orderHiredAt.toLocaleString("pt-BR")
            : ""}
        </Text>
      </View>
      ...
      <View style={[styles.infoContainer, { paddingHorizontal: 25 }]}>
        ...
        <Text style={styles.infoText}>
          {props.orderDetails
            ? props.orderDetails.orderDueDate.toLocaleString("pt-BR")
            : ""}
        </Text>
      </View>
      ...
      >
        ...
        <Text style={styles.infoText}>
          R${" "}
          {props.orderDetails
            ? props.orderDetails.orderCost.toFixed(2).replace(".", ",")
            : ""}
        </Text>
      </View>
      ...
      >
        <Text style={[styles.buttonText, styles.goBackText]}>Voltar</Text>
      </TouchableHighlight>
    </Fragment>
  );
}
...
function AdjustEditMode(props: {
  ...
  function updateOrderDetails(adjusts: AdjustData[]) {
    ...
    adjusts.forEach((adjust) => {
      ...
      if (index !== -1) {
        newAdjustList.push(orderDetails.items[itemIndex].adjusts[index]);
      } else {
        newAdjustList.push({
  ...
  function handleSaveModifications() {
    ...
    database.transaction((transaction) => {
      const orderTotalCost = orderDetails!.items.reduce((accumulator, item) => {
        return (
          accumulator +
  ...
  return (
    <Fragment>
      ...
      <View style={[styles.dueDateWrapper, { paddingHorizontal: 25 }]}>
        ...
        >
          <Text style={styles.dueDateValue}>
            {orderDetails
              ? orderDetails.orderDueDate.toLocaleDateString("pt-BR")
              : ""}
          </Text>
      ...
      <Text
        style={[styles.serviceCountForDueDateText, { paddingHorizontal: 25 }]}
      >
      ...
      {isModalOpen && (
        <AdjustListModal
          ...
          openedItem={
            orderItemsState
              ? orderItemsState.findIndex((state) => state === true)
              : -1
          }
          ...
          (_, resultSet) => {
            if (resultSet.rows.length > 0) {
                ...
                resultSet.rows._array.map((rawData) => {
                  return {
                    ...
                    value: rawData.measure_value
                      ? String(
                          rawData.measure_value.toFixed(2).replace(".", ",")
                        )
                      : "0",
                  };
                });
              ...
              setOrderDetails({
                ...
                hiredAt: new Date(resultSet.rows.item(0).created_at),
                orderItemId: resultSet.rows.item(0).order_item_id,
                orderItemTitle: resultSet.rows.item(0).order_item_title,
                orderItemDescription:
                  resultSet.rows.item(0).order_item_description,
                dueDate: new Date(resultSet.rows.item(0).due_date),
...
function TailoredDetailMode({
  ...
  function handleUpdateMeasureItem(index: number, newValue: string) {
    setOrderDetails((prevState) => {
      ...
      const updatedCustomerMeasurementList = customerMeasurementListCopy.map(
  ...
  return (
    <Fragment>
      ...
      <View style={styles.infoContainer}>
        ...
        <Text style={styles.infoText}>{orderDetails?.orderItemTitle}</Text>
      </View>
    ...
    database.transaction((transaction) => {
      transaction.executeSql(
        `
                ...
                WHERE id = ?;
            `,
        [orderDetails.cost, orderDetails.dueDate.toISOString(), props.orderId],
        (transaction, resultSet) => {
          if (resultSet.rowsAffected !== 1) {
          ...
          transaction.executeSql(
            ...
            (transaction, resultSet) => {
              ...
              const measurementsToUpdate = orderDetails.measurements.filter(
                (measure) => measure.id !== null && measure.value !== null
              );

              if (measurementsToUpdate.length > 0) {
                console.log("itens para atualizar");
                measurementsToUpdate.forEach((measure) => console.log(measure));
                ...
                measurementsToUpdate.forEach((measure) => {
                  transaction.executeSql(
                    `
                                        ...
                                        WHERE id = ?;
                                    `,
                    [Number(measure.value!.replace(",", ".")), measure.id],
                    (_, resultSet) => {
                      if (resultSet.rowsAffected === 1) {
                        console.log("medida atualizada!");
                      } else {
                        console.log("deu ruim na medida!");
                      }
                    }
                  );
                });
              ...
              const measurementsToCreate = orderDetails.measurements.filter(
                (measure) => measure.id === null && measure.value !== null
              );

              if (measurementsToCreate.length > 0) {
                console.log("itens para criar");
                measurementsToCreate.forEach((measure) => console.log(measure));

                const measurementSQLValues = measurementsToCreate.map(
                  (measure) => {
                ...
                transaction.executeSql(
                  ...
                  (_, resultSet) => {
                    if (resultSet.insertId && resultSet.insertId > 0) {
                      ...
                      console.log("Deu ruim na inserção de medidas!");
                    }
                  }
                );
              }

              const measurementsToDelete = orderDetails.measurements.filter(
                (measures) => measures.id !== null && measures.value === null
              );

              if (measurementsToDelete.length > 0) {
                console.log("itens para deletar");
                measurementsToDelete.forEach((measure) => console.log(measure));

                const measurementsToDeleteSQL = measurementsToDelete
                ...
                transaction.executeSql(
                  ...
                  (_, resultSet) => {
                    if (resultSet.rowsAffected > 0) {
                      ...
                      console.log("deu ruim na exclusão!");
                    }
                  }
                );
              }
    ...
    <Fragment>
      ...
      >
        <View style={styles.updateMeasurementsContent}>
          <UpdateIcon
            ...
            height={20}
          />
          <Text style={styles.updateMeasurementsText}>Atualizar medidas</Text>
        </View>
      </TouchableHighlight>
      ...
      {isModalOpen && (
        <MeasurementListModal
          isOpened={isModalOpen}
          onCloseModalFn={closeModal}
          existentMeasurements={orderDetails ? orderDetails.measurements : []}
    ...
    useCallback(() => {
      database.transaction((transaction) => {
        transaction.executeSql(
          ...
          undefined,
          (_, resultSet) => {
            const adjusts: AdjustData[] = resultSet.rows._array.map((row) => {
              return {
                id: row.id,
